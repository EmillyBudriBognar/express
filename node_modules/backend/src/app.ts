...
/**
 * Interface para representar um estudante/usuário no banco de dados
 */
interface User extends RowDataPacket {
  id: number;
  nome: string;
  email: string | null; // Email pode ser null se não foi preenchido
}
...
export class App {
   ...
   */
  async insertQuery(nome: string, email?: string): Promise<ResultSetHeader> {
    // Validação de entrada
    ...
    try {
      ...
      const [result] = await conn.query<ResultSetHeader>(sql, [dados]);
   ...
   * @throws Error se houver falha na conexão, query ou validação
   */
  async updateQuery(id: number, nome?: string, email?: string): Promise<ResultSetHeader> {
    // Validação de entrada
    if (!id || id <= 0 || !Number.isInteger(id)) {
      throw new Error('ID inválido: deve ser um número inteiro positivo');
    }
    ...
    const dadosParaAtualizar: { nome?: string, email?: string | null } = {};
    ...
    try {
      ...
      const [result] = await conn.query<ResultSetHeader>(sql, [dadosParaAtualizar, id]);
   ...
   * @throws Error se houver falha na conexão ou query
   */
  async deleteQuery(id: number): Promise<ResultSetHeader> {
    // Validação de entrada
    if (!id || id <= 0 || !Number.isInteger(id)) {
      throw new Error('ID inválido: deve ser um número inteiro positivo');
    }
