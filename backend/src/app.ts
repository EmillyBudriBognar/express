...
export class App {
  async executeSearchQuery(): Promise<User[]> {
    let conn;
    try {
  ...
  async searchQueryById(id: number): Promise<User | null> {
    let conn;
    try {
      conn = await connection();
      let sql = "select * from estudante where id = ?";
      const [rows] = await conn.query<User[]>(sql, [id]);
  ...
  async insertQuery(nome: string, email: string): Promise<any> {
    let conn;
    try {
      conn = await connection();
  ...
  async updateQuery(id: number, nome?: string, email?: string): Promise<any> {
    let conn;
    try {
      conn = await connection();
      let dadosParaAtualizar: { nome?: string, email?: string } = {};
  ...
  async deleteQuery(id: number): Promise<any> {
    let conn;
    try {
      conn = await connection();
